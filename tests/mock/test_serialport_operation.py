...
def test_bluetooth_mapping_disabled_by_default(mock_mgr):
    ...
    mock_mgr.systemd_disable.assert_any_call('hciuart', sudo=True)
...
def test_bluetooth_mapping_enabled(mock_mgr):
    ...
    mock_mgr.systemd_enable.assert_any_call('hciuart', sudo=True)
...
def test_serial_console_apply_enabled(mock_save, mock_load, mock_mgr):
    ...
    mock_mgr.systemd_unmask.assert_any_call('serial-getty@ttyS0.service', sudo=True)
    mock_mgr.systemd_unmask.assert_any_call('serial-getty@ttyAMA0.service', sudo=True)
    mock_mgr.systemd_enable.assert_any_call('serial-getty@ttyS0.service', sudo=True)
...
def test_serial_console_apply_disabled(mock_save, mock_load, mock_mgr):
    ...
    mock_mgr.systemd_mask.assert_any_call('serial-getty@ttyS0.service', sudo=True)
    mock_mgr.systemd_mask.assert_any_call('serial-getty@ttyAMA0.service', sudo=True)
